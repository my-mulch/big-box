    ...
    for (i = 0; i < dim; i += 1) {
        ...
        if (e == 0) {
            ...
            for (ii = i + 1; ii < dim; ii += 1) {
                ...
                if (args.of.data[ii * dim + i] != 0) {
                    //it would make the diagonal have a non-0 so swap it
                    for (j = 0; j < dim; j++) {
                        e = args.of.data[i * dim + j];       //temp store i'th row
                        args.of.data[i * dim + j] = args.of.data[ii * dim + j];//replace i'th row by ii'th
                        args.of.data[ii * dim + j] = e;      //repace ii'th by temp
        ...
        for (j = 0; j < dim; j++) {
            args.of.data[i * dim + j] = args.of.data[i * dim + j] / e; //apply to original matrix
        ...
        for (ii = 0; ii < dim; ii++) {
            ...
            // algorithm correctly)
            for (j = 0; j < dim; j++) {
                args.of.data[ii * dim + j] -= e * args.of.data[i * dim + j]; //apply to original matrix
