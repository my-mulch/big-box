...
export default function () {
    ...
    const bbcs = bb.rand({ shape: [1e5, 1] })
    const jscs = new Array(1e5).fill(null).map(function (_, i) { return Complex(bbcs.data[i * 2], bbcs.data[i * 2 + 1]) })
    ...
    for (let j = 0; j < 1e3; j++)
        for (let i = 0; i < 1e5; i++) sum[0] += bbcs.data[i * 2], sum[1] += bbcs.data[i * 2 + 1]
